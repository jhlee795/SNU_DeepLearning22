...
class CUBDataset():
    def __init__(self, data_dir, transform, split='train', imsize=128, eval_mode=False, for_wrong=False):
        ...
        self.sentence = self.load_sentence(self.data_dir, self.filenames)
        ...
        for idx, sentence in enumerate(self.sentence):
            try:
                self.clip_enc.append(clip.tokenize(sentence))
            except:
                sentences = sentence.split('.')
                self.clip_enc.append(clip.tokenize(sentences[0]))


        for sentence in self.sentence:
            encoded_word = bert_tokenizer.encode_plus(
                text=sentence,
                add_special_tokens=True,
                padding='max_length',
                truncation=True
            )
            self.bert_enc.append(encoded_word['input_ids'])
            self.bert_mask.append(encoded_word['attention_mask'])
    ...
    def load_sentence(self, data_dir, filenames, for_wrong=False):
        """
        Loads 10 captions for each image of either 'train' or 'test' dataset and tokenizes each caption as words.
    ...
    def load_text_data(self, data_dir, split):
        ...
        if split == 'train':
            ...
            captions = train_captions # 문장을 쪼갠 것
            captions_ids = train_captions_ids # 단어의 id
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            clip_enc = self.clip_enc[new_sent_ix]
            bert_enc = self.bert_enc[new_sent_ix]
            bert_mask = self.bert_mask[new_sent_ix]
            ...
            clip_enc = self.clip_enc[index]
            bert_enc = self.bert_enc[index]
            bert_mask = self.bert_mask[index]
            data = {'caps': caps, 'bert_enc': bert_enc, 'bert_mask': bert_mask, 'clip_enc': clip_enc, 'cap_len': cap_len, 'cls_id': cls_id, 'key': key, 'sent_ix': sent_ix, 'cap_ix': index}
